...
from geometry_msgs.msg import PoseStamped,Transform, TransformStamped, Pose, PointStamped, Point
...
async def ros_sub_GoalPose():
    ...
    # Subscribe to the /clicked_point topic, create a goto and send by UDP to DUNE.
    node.create_subscription(PointStamped, '/clicked_point', vehicle.ros_update_GoalPose, 100)
    tf_broadcaster = TransformBroadcaster(node)
    
    while rclpy.ok():
        
        if vehicle.tf.header.frame_id != '':

            #print("[ROS Connection] Publishing Transform")
            vehicle.tf.header.stamp = node.get_clock().now().to_msg()
            tf_broadcaster.sendTransform(vehicle.tf)
...
async def udp_main():
    ...
    sub.periodic_async(vehicle.send_goal,5)
